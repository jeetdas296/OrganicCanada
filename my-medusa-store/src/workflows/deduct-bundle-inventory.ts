...
const identifyBundleItemsStep = createStep("identify-bundle-items", async (orderId: string, { container }) => {
  ...
  if (order.sales_channel_id) {
    try {
      ...
      if (sc?.stock_locations?.length) {
        const firstLocation = sc.stock_locations[0]
        if (firstLocation?.id) {
  ...
  const deductions: Array<{ product_id: string; deduct_quantity: number }> = []
    ...
    for (const variant of product.variants || []) {
      for (const ii of variant.inventory_items || []) {
        ...
        if (ii.inventory_item_id) {
          ...
          break
        } else if ((ii as any).id && !(ii as any).variant_id) {
          inventoryItemId = (ii as any).id
        }
      }
